...
import { BoardLead } from "../LeadsKanban/types";

const { block, element } = createBEMClasses("lead-detail-view");

const LeadDetailView = ({ lead }: { lead: BoardLead }) => {
  const isQuote = lead.kind === "quote";

  return (
    <Stack gap="1.5rem" alignItems="flex-start" className={block()}>
      ...
      <LeadAssignees
        leadKind={lead.kind}
        leadId={lead.data.id}
        assignedAdminIds={lead.data.assignedAdminIds}
      />
